...
class JavaCommand < Command
  ...
  def remove_command
    "sh #{@relative_path}/remove.sh #{@temp_files.get_copied_program_path} #{@temp_files.get_temp_path}/Main.class #{@temp_files.get_program_output_path} #{@temp_files.get_error_path}"
  end
